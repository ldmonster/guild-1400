...
float g_roll = 0.0f;
float g_rating = 0.0f;
float ScriptRoll() { return g_roll; }
...
TEST(WorldEconomy2E2E, AppointActDismissFlow) {
    ...
    // Six staff; four serve office 77. Holder office-id 99 (no self-exclusion).
    ...
    int deps = CountOfficeDependents(/*target*/ 77, personEntity, nullptr, present,
                                     /*holderOfficeId*/ 99, 6,
                                     /*totalAmount*/ payment * 100, &perHead);
    WorldEconomy2ResetHooks();

    CHECK_EQ(deps, 4);                          // four dependents
    CHECK_EQ(g_payCalls, 4);                    // a command per dependent
    CHECK_EQ(g_lastValue, -(payment * 100));    // commands carry -total
    CHECK_EQ(perHead, (payment * 100) / 4);     // 3600/4 = 900
