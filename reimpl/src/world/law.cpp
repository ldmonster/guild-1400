...
double GesetzComputeMaxWantedLevelFromSum(int guardStationSum) {
    // gilde.exe 0x4c2ba0: per-building loop tracks max workstation-cat-10 sum; a
    // building whose sum == 75 short-circuits to 0.75 (return 0.75), otherwise
    // returns (float)((double)maxSum * flt_61E588). The original's return value is
    // truncated to 32-bit float by the caller's `fstp [esp+var_1C]` (var_1C is a
    // `float`) before the `fcomp`, so we round-trip through float to stay bit-exact.
    if (guardStationSum == 75)
        return 0.75;
    float f = static_cast<float>(static_cast<double>(guardStationSum) * kWantedScalar);
    return static_cast<double>(f);
}
...
int GesetzEvaluateViolation(u8 lawId, int value, i32 victimId, i32 perpetratorId,
    ...
    // gilde.exe 0x4c2cd8 switch (op-1): the roll path (loc_4C2CE9, "violation") is
    // taken on these conditions — verified against the disasm jcc directions
    // (case 3 jge->no / case 4 jg->no / case 5 jle->no / case 6 jl->no).
    bool violation;
    switch (rec.op) {
    case kLawOpEqual:        violation = (value == rec.threshold); break;  // jnz def
    case kLawOpNotEqual:     violation = (value != rec.threshold); break;  // jz  def
    case kLawOpLess:         violation = (value <  rec.threshold); break;  // jge def
    case kLawOpLessEqual:    violation = (value <= rec.threshold); break;  // jg  def
    case kLawOpGreater:      violation = (value >  rec.threshold); break;  // jle def
    case kLawOpGreaterEqual: violation = (value >= rec.threshold); break;  // jl  def
    case kLawOpSpecial:
