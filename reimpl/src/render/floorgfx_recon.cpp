...
const Texture* FloorTextureResolver::LoadSlot(int slot) {
    ...
    // "*" + name + ".BMP" (asc_62959C + name + aBmp), verbatim — no season and
    // no _high/detail suffix. The engine's floor loader consumes the name it is
    // handed; the SEASONAL ground is selected upstream through the TXS material
    // sets (the F/S/H/W rows, RealTextureSource — viaTXS), NOT by appending a
    // suffix here. We load the BASE mip layer (the single bound record).
